...
// ── Types ────────────────────────────────────────────────────────────────────
type View = "landing" | "auth" | "dashboard" | "ingest" | "review" | "mapping" | "generate" | "distribute" | "settings" | "college" | "examPrep" | "courseSite" | "questionBank" | "superadmin" | "translator" | "master";
...
export default function Data360Page() {
  ...
  const [masterUnitsEdit, setMasterUnitsEdit] = useState("");
  ...
  const [collegeSuggestingSubjects, setCollegeSuggestingSubjects] = useState(false);
  const [collegeSuggesting, setCollegeSuggesting] = useState(false);
  const [collegePasteText, setCollegePasteText] = useState("");
  const [collegeBusy, setCollegeBusy] = useState(false);
  const [collegeErr, setCollegeErr] = useState("");
  const [collegeResult, setCollegeResult] = useState<StudyPack | null>(null);
  const [isPrinting, setIsPrinting] = useState(false);
  const [collegeHistory, setCollegeHistory] = useState<StudyPack[]>([]);
  const [collegeTargetLang, setCollegeTargetLang] = useState("English");
  const [collegeProvider, setCollegeProvider] = useState("");
  const [collegeCached, setCollegeCached] = useState(false);
  

  useEffect(() => {
    fetch("/api/examhub360-college/config")
      .then(r => r.json())
  ...
  const [fetchingAnswerFor, setFetchingAnswerFor] = useState<any>(null);
  const fetchInlineAnswer = async (chapterName: string, chunkType: string, q: any, i: number) => {
    // Stub function just to make TypeScript happy in College page for now
    console.log(chapterName, chunkType, q, i);
  };
  ...
  const [examPrepIncludeViva, setExamPrepIncludeViva] = useState(true);
  ...
  // ── Super Admin Config ────────────────────────────────────────────────────────
  const [adminUnitConfig, setAdminUnitConfig] = useState<Record<string, "demo" | "paid">>({});
  const [adminPaidUsers, setAdminPaidUsers] = useState<string[]>([]);

  const [adminNewPaidUser, setAdminNewPaidUser] = useState("");
  const [adminNewUnit, setAdminNewUnit] = useState("");
  ...
  useEffect(() => {
    ...
    try {
      ...
      const storedUnits = localStorage.getItem("examhub_unit_config");
      if (storedUnits) setAdminUnitConfig(JSON.parse(storedUnits));
  ...
  const handleExamPrepGenerate = async () => {
    ...
    try {
      const res = await fetch("/api/examhub360-college/generate-exam", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          ...
          includeViva: examPrepIncludeViva,
  ...
  useEffect(() => {
    ...
    const fetchSubjects = async () => {
      const cacheKey = `d360_subjects_${collegeDegree}_${collegeSemester}`;
      const cached = localStorage.getItem(cacheKey);
      if (cached) {
        try {
          setCollegeCoursesList(JSON.parse(cached));
          return;
        } catch (e) {}
      }
      setCollegeSuggestingSubjects(true);
      try {
        ...
        if (result.success && Array.isArray(result.data)) {
          setCollegeCoursesList(result.data);
          localStorage.setItem(cacheKey, JSON.stringify(result.data));
        }
      } catch (e) {
        // silently fail and fallback to empty/custom input
      } finally {
        setCollegeSuggestingSubjects(false);
      }
    };
    fetchSubjects();
  ...
  useEffect(() => {
    if (view === "master") {
      const cacheKey = `d360_subjects_${masterDegree}_${masterSemester}`;
      const cached = localStorage.getItem(cacheKey);
      if (cached) {
  ...
  }, [masterDegree, masterSemester, view]);

  useEffect(() => {
    if (view === "master" && masterCourse) {
      const cacheKey = `d360_units_${masterDegree}_${masterSemester}_${masterCourse}`;
      const cached = localStorage.getItem(cacheKey);
      if (cached) {
        try { 
          const units = JSON.parse(cached);
          const parsed = Array.isArray(units) ? units : (units.units || []);
          setMasterUnitsEdit(parsed.join("\n")); 
        } catch (e) { setMasterUnitsEdit(""); }
      } else {
        setMasterUnitsEdit("");
      }
    }
  }, [masterDegree, masterSemester, masterCourse, view]);

  const filterUnits = (units: string[]) => {
    const isUserPaid = isSuperadmin || (user?.email && adminPaidUsers.includes(user.email.toLowerCase()));
    if (isUserPaid) return units;
    return units.filter(chap => adminUnitConfig[chap] !== "paid");
  };
  ...
  const suggestUnits = async () => {
    ...
    try {
      const res = await fetch("/api/examhub360-college/suggest-units", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
      ...
      const subjectsCacheKey = `d360_subjects_${collegeDegree}_${collegeSemester}`;
      let masterSubjects = [];
      try { masterSubjects = JSON.parse(localStorage.getItem(subjectsCacheKey) || "[]"); } catch (e) {}
      if (!masterSubjects.includes(collegeCourse)) {
        masterSubjects.push(collegeCourse);
        localStorage.setItem(subjectsCacheKey, JSON.stringify(masterSubjects));
        setCollegeCoursesList(masterSubjects);
      }
    } catch (error: any) {
      setCollegeErr(error.message || "Failed to suggest units");
    } finally {
      setCollegeSuggesting(false);
    }
  };
  ...
  const runStudyGuideFromFiles = async (fileList: FileList) => {
    if (!collegeSemester.trim() || !collegeDegree.trim()) {
      setCollegeErr("Class and Degree are required first.");
      return;
    }
    setCollegeBusy(true); setCollegeErr(""); setCollegeResult(null);
    try {
      const images: { image_base64: string; mime_type: string }[] = [];
  ...
  const runStudyGuideGenerate = async () => {
    if (!collegeSemester.trim() || !collegeDegree.trim() || !collegeCourse.trim() || !collegeUnitLabel.trim()) {
      setCollegeErr("Class, Degree, Subject, and Unit Name are required to generate from scratch.");
      return;
    }
    setCollegeBusy(true); setCollegeErr(""); setCollegeResult(null);
    try {
  ...
  const runBatchGeneration = async (forceBypass = false) => {
    if (collegeSelectedUnits.length === 0) {
    ...
    const results: CourseUnitData[] = [];
    
    // Load DB to check for cached units
    const existingDbStr = localStorage.getItem("examhub_saved_units");
    let existingDb: CourseUnitData[] = [];
    if (existingDbStr) {
      try { existingDb = JSON.parse(existingDbStr); } catch(e){}
    }

    try {
      for (let i = 0; i < collegeSelectedUnits.length; i++) {
        const chap = collegeSelectedUnits[i];
        
        // Check DB first unless overridden
        const forceGenerate = (forceBypass === true) || examPrepPromptOverride.includes("***Generate New");
        const cachedUnit = existingDb.find(c => c.unit === chap);
        // Only require studyPack. examPrep might legitimately be null if skipped or failed previously.
        if (!forceGenerate && cachedUnit && cachedUnit.studyPack) {
          setBatchProgress({ current: i + 1, total: collegeSelectedUnits.length, status: `Loading "${chap}" from Database...` });
          results.push(cachedUnit);
          // artificial delay for UI
          await new Promise(r => setTimeout(r, 500));
          continue;
        }
        
        setBatchProgress({ current: i + 1, total: collegeSelectedUnits.length, status: `Generating Core Concepts for "${chap}" (1/3)...` });
        ...
        let examPrep = null;
        try {
          const examRes = await fetch("/api/examhub360-college/generate-exam", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
        ...
        results[results.length - 1].examPrep = examPrep;
      }
      ...
      for (const res of results) {
        const idx = existingDb.findIndex(c => c.unit === res.unit);
        if (idx >= 0) existingDb[idx] = res;
        else existingDb.push(res);
      }
      localStorage.setItem("examhub_saved_units", JSON.stringify(existingDb));
      localStorage.setItem("d360_courseSiteData", JSON.stringify(results)); // Keep legacy key for current session
    } catch (e: any) {
      setCollegeErr(e.message || "Failed to generate batch course site.");
    } finally {
      setCollegeBusy(false);
  ...
  useEffect(() => {
    document.title = "College360 — Autonomous Exam Prep & Study Generation";
    if (getToken()) {
  ...
  const handleAuth = async () => {
    ...
    try {
      ...
      if (authPortal === "school") {
        window.location.href = "/examhub360";
        return;
      }
        ...
        try {
          await fetch("/api/send-email", {
            ...
            body: JSON.stringify({
              ...
              subject: "New College360 Registration",
  ...
  const saveMasterSubjects = () => {
    ...
    localStorage.setItem(`d360_subjects_${masterDegree}_${masterSemester}`, JSON.stringify(list));
    setMasterMessage("Subjects saved successfully to cache.");
    setTimeout(() => setMasterMessage(""), 3000);
  };

  const saveMasterUnits = () => {
    const list = masterUnitsEdit.split("\n").map(s => s.trim()).filter(s => s);
    localStorage.setItem(`d360_units_${masterDegree}_${masterSemester}_${masterCourse}`, JSON.stringify(list));
    setMasterMessage("Units saved successfully to cache.");
    setTimeout(() => setMasterMessage(""), 3000);
  };
    ...
    <div className="min-h-screen bg-slate-50 font-sans">
      ...
      <nav className="bg-white border-b border-gray-200 sticky top-0 z-30">
        <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between gap-4">
          <button onClick={() => go(user ? "college" : "landing")} className="flex items-center gap-2 shrink-0">
            ...
            <div className="text-left">
              <p className="font-black text-gray-900 text-sm leading-none">College360</p>
          ...
          {user && (
            <div className="hidden md:flex items-center gap-6 text-sm font-medium text-gray-600">
              ...
              <Link href="/examhub360" className="hover:text-teal-600 transition">School / Study</Link>
          ...
          <div className="bg-white border border-gray-200 rounded-2xl p-6 space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-4 gap-3">
              ...
              <div>
                ...
                <select value={collegeTargetLang} onChange={e => setCollegeTargetLang(e.target.value)} className="mt-1 w-full border border-gray-200 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:border-teal-400">
            ...
            {collegeUnitsList.length > 0 && (
              <div className="mt-4 border-t border-gray-100 pt-4">
                <h3 className="text-sm font-bold text-gray-800 mb-3">Select Chapters to Generate ({collegeSelectedUnits.length} selected)</h3>
                ...
                <div className="flex justify-end gap-3 mt-4 pt-3 border-t border-gray-100">
                  <button onClick={() => runBatchGeneration(true)} disabled={collegeBusy || collegeSelectedUnits.length === 0} className="h-10 flex items-center justify-center gap-2 bg-white border border-teal-600 text-teal-700 hover:bg-teal-50 disabled:opacity-40 font-bold text-sm px-6 rounded-lg transition shadow-sm">
                    <Sparkles size={16} /> Regenerate (Bypass Cache)
                  </button>

                    <button onClick={() => runBatchGeneration(false)} disabled={collegeBusy} className="h-10 flex items-center justify-center gap-2 bg-gradient-to-r from-teal-600 to-teal-700 hover:from-teal-700 hover:to-teal-800 text-white disabled:opacity-40 font-bold text-sm px-6 rounded-lg transition shadow-sm">
                      {collegeBusy ? <Loader2 size={16} className="animate-spin" /> : <GraduationCap size={16} />} 
                      {collegeBusy ? "Batch Generating..." : `Generate Course Site (${collegeSelectedUnits.length} Units)`}
                    </button>
                  </div>
                  {batchProgress && (
          ...
          <div className="bg-white border border-gray-200 rounded-2xl p-6 space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="border border-gray-200 rounded-xl p-4 bg-white">
                ...
                <textarea value={examPrepPromptOverride} onChange={e => setExamPrepPromptOverride(e.target.value)} rows={4} placeholder="e.g. Ensure all questions are strictly aligned to the latest Lab Viva competency framework..." className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-teal-400 resize-none" />
              </div>
            </div>
        ...
        <div className="flex h-[calc(100vh-64px)] overflow-hidden bg-gray-50">
          ...
          <div className="w-72 bg-white border-r border-gray-200 overflow-y-auto flex flex-col">
            <div className="p-4 border-b border-gray-200 sticky top-0 bg-white z-10 shadow-sm">
              ...
              {batchProgress && (
                <div className="mt-3 p-2.5 bg-teal-50 rounded-lg border border-teal-100 flex flex-col gap-1.5 animate-pulse">
                  <div className="flex justify-between items-center text-[10px] font-bold text-teal-800">
                    <span className="truncate pr-2">{batchProgress.status}</span>
                    <span className="shrink-0">{batchProgress.current} / {batchProgress.total}</span>
                  </div>
                  <div className="w-full bg-teal-200 rounded-full h-1">
                    <div className="bg-teal-600 h-1 rounded-full transition-all duration-300" style={{ width: `${(batchProgress.current / batchProgress.total) * 100}%` }}></div>
                  </div>
                </div>
              )}
            </div>
              ...
              {courseSiteData.map((data, idx) => (
                <div key={data.unit}>
                  ...
                  >
                    ...
                    {courseActiveUnit === data.unit ? <ChevronRight size={14} className="text-teal-500 rotate-90 transition-transform" /> : <ChevronRight size={14} className="text-gray-400 group-hover:text-teal-500 opacity-0 group-hover:opacity-100 transition" />}
                  </button>
...
<button onClick={() => setCourseActiveTab("mistakes")} className={`w-full text-left px-2 py-1.5 text-[11px] font-bold rounded transition ${(isPrinting || courseActiveTab === "mistakes") ? "text-teal-700 bg-teal-50" : "text-gray-500 hover:bg-gray-100 hover:text-gray-700"}`}>Common Mistakes to Avoid</button>
                      ...
                      {data.studyPack?.lab_viva_questions && data.studyPack.lab_viva_questions.length > 0 && (
                        <button onClick={() => setCourseActiveTab("viva")} className={`w-full text-left px-2 py-1.5 text-[11px] font-bold rounded transition ${courseActiveTab === "viva" ? "text-teal-700 bg-teal-50" : "text-gray-500 hover:bg-gray-100 hover:text-gray-700"}`}>Viva Questions</button>
                      )}
              ...
              {courseSiteData.filter(d => d.unit === courseActiveUnit).map(data => (
                <div key={data.unit}>
                  ...
                  <div className="border-b border-gray-200 pb-6 mb-8">
                    <h1 className="text-3xl font-black text-gray-900">{data.unit}</h1>
                    
                    <div className="flex gap-3 mt-3">
                      <span className="bg-gray-200 text-gray-700 text-[10px] font-bold px-2 py-1 rounded uppercase tracking-wide">Chapter</span>
                      <span className="bg-teal-100 text-teal-800 text-[10px] font-bold px-2 py-1 rounded uppercase tracking-wide">{collegeCourse}</span>
                          ...
                          <details key={i} open={isPrinting} className="group border border-gray-200 rounded-xl bg-white overflow-hidden shadow-sm">
                            ...
                            <div className="p-4 pt-0 text-sm text-gray-700 bg-gray-50 border-t border-gray-100 mt-2">
                              ...
                              {q.answer ? (
                                ...
                                <button 
                                  onClick={() => fetchInlineAnswer(data.unit, courseActiveTab, q, i)}
                                  disabled={fetchingAnswerFor?.chapter === data.unit && fetchingAnswerFor?.type === courseActiveTab && fetchingAnswerFor?.index === i}
                                ...
                                >
                                  {fetchingAnswerFor?.chapter === data.unit && fetchingAnswerFor?.type === courseActiveTab && fetchingAnswerFor?.index === i ? "Generating Answer..." : "Get Answer with AI"}
                                </button>
                              )}
                  ...
                  {/* Lab Viva Questions */}
                  {courseActiveTab === "viva" && data.studyPack?.lab_viva_questions && data.studyPack.lab_viva_questions.length > 0 && (
                    <section className="mb-10">
                      <h2 className="text-xl font-black text-gray-900 mb-4 flex items-center gap-2"><BookOpen size={20} className="text-teal-500" /> Lab Viva Questions & Answers</h2>
                      <div className="space-y-3">
                        {data.studyPack.lab_viva_questions.map((q: any, i: number) => (
                          <details key={i} open={isPrinting} className="group border border-gray-200 rounded-xl bg-white overflow-hidden shadow-sm">
                            <summary className="font-medium text-sm text-gray-900 p-4 cursor-pointer hover:bg-gray-50 transition list-none flex gap-2">
                              <span className="text-teal-600 font-black">Lab Viva Q{i+1}.</span> {q.question}
                            </summary>
                            <div className="p-4 pt-0 text-sm text-gray-700 bg-gray-50 border-t border-gray-100 mt-2">
                              <span className="font-bold text-teal-800 block text-[10px] uppercase tracking-wide mb-1">Answer</span>
                              {q.answer ? (
        ...
        <div className="max-w-4xl mx-auto px-4 py-8">
          <div className="flex items-center justify-between mb-8">
            <div>
              ...
              <p className="text-sm text-gray-500 mt-1">Manage paid users and unit access rules.</p>
            </div>
          </div>
          ...
          <div className="grid gap-6">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <button 
                    onClick={() => {
                      ...
                      setGlobalConfig(nu);
                      fetch("/api/examhub360-college/config", { method: "POST", body: JSON.stringify(nu) });
                    }}
            ...
            <div className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm">
              <h3 className="font-bold text-gray-900 border-b border-gray-100 pb-2 mb-4">Unit Access Configuration</h3>
              <p className="text-xs text-gray-500 mb-4">Define which units require paid status. By default, units are considered Demo (free) unless explicitly marked here as Paid.</p>
              
              <div className="flex gap-2 mb-4">
                <input value={adminNewUnit} onChange={e => setAdminNewUnit(e.target.value)} placeholder="Exact Unit Name (e.g. Current Electricity)" className="flex-1 text-sm border border-gray-200 rounded-lg px-3 py-2 focus:outline-none focus:border-teal-400" />
                <button 
                  onClick={() => {
                    if (!adminNewUnit) return;
                    const nc = { ...adminUnitConfig, [adminNewUnit.trim()]: "paid" as const };
                    setAdminUnitConfig(nc);
                    localStorage.setItem("examhub_unit_config", JSON.stringify(nc));
                    setAdminNewUnit("");
                  }}
                  className="bg-teal-600 hover:bg-teal-700 text-white font-bold text-sm px-4 py-2 rounded-xl transition"
                >
              ...
              <ul className="space-y-2">
                {Object.entries(adminUnitConfig).map(([chap, type]) => (
                ...
                {Object.keys(adminUnitConfig).length === 0 && <li className="text-xs text-gray-400 py-2">No units locked yet.</li>}
              </ul>
            </div>
          </div>
        </div>
