  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      <div className="flex items-center gap-3 mb-6">
        <div className="w-10 h-10 bg-gradient-to-br from-blue-500/20 to-blue-600/10 border border-blue-500/20 rounded-xl flex items-center justify-center shadow-sm shadow-blue-950/20"><ShieldCheck size={18} className="text-blue-400" /></div>
        <div><h1 className="text-xl font-bold text-white tracking-tight">Compliance & Facility</h1><p className="text-white/40 text-xs">Licenses, renewals, and utility usage — one calendar across vehicles, machines, workers, facility</p></div>
      </div>
      ...
      {tab === "calendar" && (
        <>
          <button onClick={getAiSummary} disabled={loadingAi} className="mb-4 flex items-center gap-1 bg-gradient-to-r from-purple-600/25 to-fuchsia-600/25 hover:from-purple-500/30 hover:to-fuchsia-500/30 text-purple-200 border border-purple-500/30 rounded-lg text-sm font-medium px-4 py-2 disabled:opacity-50">
      ...
      {tab === "facilities" && (
        <>
          <div className="bg-gradient-to-b from-[#181c26] to-[#12151b] border border-white/10 rounded-2xl shadow-lg shadow-black/20 p-4 mb-4 grid grid-cols-2 sm:grid-cols-5 gap-2">
            <input placeholder="Name" value={facForm.name} onChange={e => setFacForm({ ...facForm, name: e.target.value })} className="bg-[#0d0f16] border border-white/10 rounded-lg focus:border-green-500/40 focus:ring-1 focus:ring-green-500/20 transition-colors px-3 py-2 text-sm text-white" />
            <select value={facForm.type} onChange={e => setFacForm({ ...facForm, type: e.target.value })} className="bg-[#0d0f16] border border-white/10 rounded-lg focus:border-green-500/40 focus:ring-1 focus:ring-green-500/20 transition-colors px-3 py-2 text-sm text-white">
              {["pollution_control", "fssai", "factory_license", "fire_safety", "other"].map(t => <option key={t} value={t}>{t.replace(/_/g, " ")}</option>)}
            </select>
            <input placeholder="Authority" value={facForm.authority} onChange={e => setFacForm({ ...facForm, authority: e.target.value })} className="bg-[#0d0f16] border border-white/10 rounded-lg focus:border-green-500/40 focus:ring-1 focus:ring-green-500/20 transition-colors px-3 py-2 text-sm text-white" />
            <input type="date" value={facForm.renewal_date} onChange={e => setFacForm({ ...facForm, renewal_date: e.target.value })} className="bg-[#0d0f16] border border-white/10 rounded-lg focus:border-green-500/40 focus:ring-1 focus:ring-green-500/20 transition-colors px-3 py-2 text-sm text-white" />
            <button onClick={addFacility} className="flex items-center justify-center gap-1 bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-500 hover:to-emerald-500 shadow-md shadow-green-950/40 transition-all text-white rounded-lg text-sm font-medium"><Plus size={14} /> Add</button>
          </div>
          <div className="bg-gradient-to-b from-[#181c26] to-[#12151b] border border-white/10 rounded-2xl shadow-lg shadow-black/20 overflow-hidden">
      ...
      {tab === "utilities" && (
        <>
          <div className="bg-gradient-to-b from-[#181c26] to-[#12151b] border border-white/10 rounded-2xl shadow-lg shadow-black/20 p-4 mb-4 grid grid-cols-2 sm:grid-cols-5 gap-2">
            <select value={utilForm.type} onChange={e => setUtilForm({ ...utilForm, type: e.target.value })} className="bg-[#0d0f16] border border-white/10 rounded-lg focus:border-green-500/40 focus:ring-1 focus:ring-green-500/20 transition-colors px-3 py-2 text-sm text-white">
              {["electricity", "water", "diesel_generator"].map(t => <option key={t} value={t}>{t.replace(/_/g, " ")}</option>)}
            </select>
            <input type="date" value={utilForm.usage_date} onChange={e => setUtilForm({ ...utilForm, usage_date: e.target.value })} className="bg-[#0d0f16] border border-white/10 rounded-lg focus:border-green-500/40 focus:ring-1 focus:ring-green-500/20 transition-colors px-3 py-2 text-sm text-white" />
            <input type="number" placeholder="Units consumed" value={utilForm.units_consumed} onChange={e => setUtilForm({ ...utilForm, units_consumed: e.target.value })} className="bg-[#0d0f16] border border-white/10 rounded-lg focus:border-green-500/40 focus:ring-1 focus:ring-green-500/20 transition-colors px-3 py-2 text-sm text-white" />
            <input type="number" placeholder="Cost ₹" value={utilForm.cost} onChange={e => setUtilForm({ ...utilForm, cost: e.target.value })} className="bg-[#0d0f16] border border-white/10 rounded-lg focus:border-green-500/40 focus:ring-1 focus:ring-green-500/20 transition-colors px-3 py-2 text-sm text-white" />
            <button onClick={addUtility} className="flex items-center justify-center gap-1 bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-500 hover:to-emerald-500 shadow-md shadow-green-950/40 transition-all text-white rounded-lg text-sm font-medium"><Zap size={14} /> Log</button>
          </div>
          <div className="bg-gradient-to-b from-[#181c26] to-[#12151b] border border-white/10 rounded-2xl shadow-lg shadow-black/20 overflow-hidden">
